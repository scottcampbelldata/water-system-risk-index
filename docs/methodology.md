...
| Vulnerability | 0.20 | County-level CDC/ATSDR SVI percentile fallback in the MVP. Suppressed/missing SVI (CDC `-999` sentinel) is treated as missing, not as zero vulnerability. |
...
| Data quality penalty | -0.05 | Negative weight: poorly-documented systems are *deprioritized* so the screen does not over-rank them on uncertain data. It is therefore never reported as a top "risk driver." The six positive weights sum to 1.0, so the effective score ceiling is ~95. |
