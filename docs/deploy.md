...
- **Frontend** — static bundle in `web/`, hosted on **Cloudflare Pages** at
  `https://water-risk.scottcampbell.io`.
- **Backend** — FastAPI + Postgres on the VPS at
  `https://water-api.scottcampbell.io`, fronted by nginx + certbot TLS,
...
- Repo checked out at `/home/scott/water-system-risk-index`.

### Install dependencies
```bash
cd /home/scott/water-system-risk-index
...
# WATER_API_PORT=8000, WATER_CORS_ORIGINS=https://water-risk.scottcampbell.io
```
...
### Reverse proxy + TLS
```bash
sudo cp deploy/nginx/water-api.scottcampbell.io.conf /etc/nginx/sites-available/
sudo ln -s /etc/nginx/sites-available/water-api.scottcampbell.io.conf /etc/nginx/sites-enabled/
sudo nginx -t && sudo systemctl reload nginx
sudo certbot --nginx -d water-api.scottcampbell.io   # adds HTTPS + redirect
```
...
- **Production** — set it to `https://water-api.scottcampbell.io` before/at deploy.
...
build step, e.g.:
```bash
echo 'window.APP_CONFIG = { apiBase: "https://water-api.scottcampbell.io" };' > web/config.js
```
