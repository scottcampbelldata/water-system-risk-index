...
hosted on Cloudflare Pages (`water-risk.scottcampbell.io`). It no longer bundles
any data file: all data is fetched from the FastAPI backend
(`water-api.scottcampbell.io`), with server-side filtering, sorting and
pagination.
...
window.APP_CONFIG = { apiBase: "https://water-api.scottcampbell.io" }; // prod
```
